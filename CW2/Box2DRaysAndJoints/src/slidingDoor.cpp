...
SlidingDoor::SlidingDoor(b2World * world, sf::Vector2f position, float angle)
{
	...
	l_shape.SetAsBox(1.2f * 0.5f, 0.16f * 0.5f, b2Vec2(0.3f, 0.f), 0.f);
	...
	l_shape.SetAsBox(1.2f * 0.5f, 0.16f * 0.5f, b2Vec2(1.5f, 0.f), 0.f);
	m_rightDoor->CreateFixture(&l_fixtureDef);
	l_shape.SetAsBox(0.7f * 0.5f, 0.04f * 0.5f, b2Vec2(2.7f, -0.10f), 0.f);
	m_rightSide->CreateFixture(&l_fixtureDef);
	l_shape.SetAsBox(0.7f * 0.5f, 0.04f * 0.5f, b2Vec2(2.7f, 0.10f), 0.f);
	m_rightSide->CreateFixture(&l_fixtureDef);
	l_shape.SetAsBox(0.18f * 0.5f, 0.18f * 0.5f, b2Vec2(2.96f, 0.f), 0.f);
	m_rightSide->CreateFixture(&l_fixtureDef);
	...
	//!<Sets the size and position fro the doors and sides
	...
	m_side2.setPosition(sf::Vector2f(m_leftSide->GetPosition().x + 2.7, m_leftSide->GetPosition().y));
	...
	m_door1.setSize(sf::Vector2f(1.2f, 0.16f));
	...
	m_door2.setSize(sf::Vector2f(1.2f, 0.16f));
...
{
	//!<Used for when the door opens setting the distance it moves based on distance and time
	if (m_state == OPENING)
	{
	...
	//!<Used for when the door closes setting the distance it moves based on distance and time
	if (m_state == CLOSING)
	{
		m_fElapsedTime += timestep;
		float t = m_fElapsedTime / m_fMotionTime;
...
//!< Draws all of the parts of the door
